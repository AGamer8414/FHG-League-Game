...
# Makes internal changes before the end
    def upon_after_loop
        ...
        if @@hash["commands"]
            $game_variables[@@hash["cmd_variable"]] = pbShowCommands(
                @@hash["msg_window"],
                @@hash["commands"],
                @@hash["cmd_if_cancel"]
            )
...
# Disposes Gold Windows etc.
    def dispose_windows
        for sprite_name, sprite in @@hash
            next unless sprite_name.starts_with?("windows")
